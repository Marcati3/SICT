...
Uses Ollama local embeddings (nomic-embed-text) to enable semantic matching
between task descriptions and heuristics. Falls back to OpenAI if available,
or keyword matching as final fallback.
...
# Cache version - increment when embedding model changes to invalidate old cache
CACHE_VERSION = 2
...
class SemanticSearcher:
    ...
    1. Ollama with nomic-embed-text (local, fast, no API calls) - PRIMARY
    2. OpenAI embeddings (if API key available) - FALLBACK
    3. Simple keyword matching (always works) - FINAL FALLBACK
    ...
    async def _initialize_model(self, prefer_openai: bool = False):
        ...
        if ollama_available():
            self._embedder = OllamaEmbedder(model=self.model_name)
        ...
        print("Warning: No embedding backend available (Ollama or OpenAI). Using keyword fallback.")

    def _get_cache_file_path(self) -> Path:
        """Get versioned cache file path."""
        return self.cache_path / f'heuristic_embeddings_v{CACHE_VERSION}.json'
    ...
    def _cleanup_old_cache(self):
        ...
        # Old unversioned cache file
        old_cache = self.cache_path / 'heuristic_embeddings.json'
        if old_cache.exists():
            try:
                old_cache.unlink()
            except Exception:
        ...
        for i in range(1, CACHE_VERSION):
            old_versioned = self.cache_path / f'heuristic_embeddings_v{i}.json'
            if old_versioned.exists():
                try:
                    old_versioned.unlink()
                except Exception:
                    pass
    ...
    async def embed(self, text: str) -> np.ndarray:
        ...
        # Generate embedding
        if self._use_ollama and self._embedder:
    ...
    async def _embed_ollama(self, text: str) -> np.ndarray:
        """Generate embedding using Ollama."""
        result = await self._embedder.embed_async(text)
        if result:
            return np.array(result)
        # Ollama failed, fall back to keyword
